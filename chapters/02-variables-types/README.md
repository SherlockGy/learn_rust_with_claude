...
上一章我们用 `let args: Vec<String> = env::args().collect();` 获取命令行参数，你可能注意到：

- 我们用 `let` 声明变量，但没有用 `=` 修改过它的值
- 代码中出现了 `Vec<String>` 这个类型标注
- 字符串似乎有 `String` 和 `"-n"` 这种字面量两种形式

这些都不是偶然——**Rust 的变量默认不可变**，**字符串有两种主要类型**。本章将深入解释这些设计背后的原因。
...
- `lines()`：返回按行分割的迭代器（复数形式通常暗示返回迭代器，如 `chars()`、`bytes()`）

**为什么需要 `lock()`？**

标准输入在多线程环境中是共享资源，`lock()` 获取独占访问。单线程程序中这是惯用写法。

**对比 Java**：Java 的 `Scanner` 是你创建的独立对象，不需要锁。但 Rust 的 `stdin()` 返回的是全局共享的句柄——操作系统层面，每个进程只有一个 stdin。Rust 的类型系统诚实地反映这一事实，要求你显式获取锁。
...
**为什么 `lines()` 返回迭代器而不是 `Vec<String>`？**

因为 stdin 可能是无限的——来自管道的数据流、持续产生的日志、永不结束的输入。如果要先读完所有行再返回 Vec，程序可能永远阻塞或内存爆炸。迭代器让你一行一行处理，按需读取。
